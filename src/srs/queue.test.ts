...
describe("assembleQueue", () => {
  ...
  it("includes a ref item only at stage >= 2, riding a due text item", () => {
    const notYetRung2 = withProgress({ f1: { stage: 1, due: 0 } });
    expect(
      assembleQueue("alpha", notYetRung2, FIXTURE_VERSES, NOW, mulberry32(1)).some((i) => i.kind === "ref"),
    ).toBe(false);

    const ready = withProgress({ f1: { stage: 2, due: 0 } });
    expect(
...
describe("isHeld", () => {
  it("is true once the word ladder reaches rung 5", () => {
    expect(isHeld({ stage: 5 })).toBe(true);
    expect(isHeld({ stage: 4 })).toBe(false);
  });
});
